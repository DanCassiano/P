...
class Git {
	...
	public static function windows_mode() {
		self::set_bin('C:\Git\bin\git.exe ');
	}
...
class GitRepo {
	 ...
	 * @return string
	 */
	public function status($html = false, $optExtras = "", $array = false) {
		$msg = $this->run("status {$optExtras}");
		if ($html == true && $array == false) {
			$msg = str_replace("\n", "<br />", $msg);
		}
		...
		{
			$msg = explode("\n", $msg);
			$msg = array_filter($msg, function($item){
				return $item !== ""? true : false;
			});

			$temp = array();
			foreach ($msg as $key => $v) {
				$temp[$key]['status'] = substr($v, 0, 2 );
				$temp[$key]['arq'] = substr($v, 2);
			}
			$msg = $temp;
		}
